...
exports.run = (client, message, args) => {
  ...
  .addField(':warning: Uyarı :warning:', '`srolver` adlı komutu özel mesajlarda kullanamazsın.')
  return message.author.sendEmbed(ozelmesajuyari); }
  let guild = message.guild;
  let user = message.mentions.users.first();
  let modlog = guild.channels.find('name', 'mod-log');
  let verilecek = args.slice(1).join(' ');
  let muteRole = client.guilds.get(message.guild.id).roles.find('name', verilecek);
  if (!modlog) return message.reply('`mod-log` kanalını bulamıyorum.').catch(console.error);
  if (!muteRole) return message.reply(verilecek +  'adlı bir rol bulamıyorum.').catch(console.error);
